...
db.once('open', function callback() {
    ...
    wines.forEach(function(wine) {
        console.log('attempt to insert wine ' + wine.name)

        var wineModel = new Wine(wine);

        wineModel.save(function(err, item) {
            numSaved++;

            if (!err) console.log("Inserted wine document: " + item);
            else console.error(err);

            if (numSaved == wines.length) {
                db.close();
            }
        });
    });
